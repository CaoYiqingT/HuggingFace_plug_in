...
class MultitaskOutput(ModelOutput):
    ...
    Args:
        - bert_sequence_output (`torch.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`):
        ...
        - attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            heads.
    """
    bert_sequence_output: torch.FloatTensor = None
...
class BertForMultipleTask(BertPreTrainedModel):
    """
    Multitask Bert Model, support the following tasks:
        - PreTraining
        - MLM
    ...
    def __init__(self, config, *multitask, **task_specific_param):
        ...
        if "PreTraining" in self.multitask:
            self.pooler = BertPooler(config)
        if "PreTraining" in self.multitask:
            assert "MLM" not in self.multitask, "PreTraining including MLM, thus MLM is not needed, please REMOVE MLM from multitasks!"
            self.pretraining_cls = BertPreTrainingHeads(config)
        ...
        if "SequenceClassification" in self.multitask:
            ...
            self.sequence_classification_classifier = BertClassificationHead(config, self.sequence_classification_num_labels)
    ...
    def get_output_embeddings(self):
        return self.mlm_cls.predictions.decoder if "MLM" in self.multitask else None
    ...
    def forward(
        ...
        if "MLM" in self.multitask:
            mlm_prediction_scores = self.mlm_cls(sequence_output)
        ...
        return MultitaskOutput(
            bert_sequence_output=sequence_output,
            loss=multitask_loss,
